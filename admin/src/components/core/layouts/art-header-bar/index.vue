...
<template>
  ...
  >
    ...
    <!-- 通知 -->
    <ArtNotification v-model:value="showNotice" ref="notice" @updated="loadNotificationBadge" />
  </div>
</template>
...
<script setup lang="ts">
  ...
  const notificationHasUnread = ref(false)
  const notice = ref(null)

  async function loadNotificationBadge() {
    try {
      const list = await request.get<Array<{ readAt?: string | null }>>({
        url: '/v1/notifications'
      })
      notificationHasUnread.value = list.some((item) => !item.readAt)
    } catch {
      notificationHasUnread.value = false
    }
  }
