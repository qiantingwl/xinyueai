  ...
  <div class="flex-cb max-md:!block" id="art-table-header">
    ...
    <div class="flex-c md:justify-end max-md:mt-3 max-sm:!hidden">
      ...
      <ElDropdown v-if="shouldShow('size')" @command="handleTableSizeChange">
        <button type="button" class="button" aria-label="表格密度" title="表格密度">
      ...
      <button
        ...
        type="button"
        class="button"
        :aria-label="isFullScreen ? '退出全屏' : '全屏显示表格'"
      ...
      <!-- 列设置 -->
      <ElPopover v-if="shouldShow('columns') && columns.length" placement="bottom" trigger="click">
        <template #reference>
