...
<template>
  <div class="mx-auto pt-5 mb-5">
    <!-- 标题 -->
    <h3 class="text-2xl font-medium text-g-900 mb-8">{{ xt('更新日志') }}</h3>
    ...
    <div class="space-y-5">
      ...
      >
        ...
        <!-- 标题 -->
        <h4 class="text-lg font-medium text-g-900 mb-3">{{ xt(item.title) }}</h4>
        ...
        <ul v-if="item.detail?.length" class="space-y-2 mb-4">
          ...
          >
            ...
            <span class="text-g-700">{{ xt(detail) }}</span>
          </li>
        </ul>
