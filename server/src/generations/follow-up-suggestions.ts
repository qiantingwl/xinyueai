/**
 * 追问建议的服务端实现。前端 `src/utils/follow-up-suggestions.ts` 有一份等价实现，
 * 用于在流式回答结束前先本地给出建议；两处必须保持一致的启发式，
 * 由 `tests/unit/follow-up-suggestions-parity.test.ts` 守住。
 *
 * 两个工程的 tsconfig 互不覆盖，物理合并需要改 rootDir 或引入 workspace 包，
 * 因此这里保留两份实现 + 一致性测试，而不是为 25 行正则改动构建结构。
 */
function normalizeQuestion(value: string) {
  return value.toLowerCase().replace(/[\s，。！？,.!?；;：:“”"'‘’]/g, '')
}

export function followUpSuggestions(prompt: string, answer: string, provided: string[] = []) {
