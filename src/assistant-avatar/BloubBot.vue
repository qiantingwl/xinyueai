...
import { blockAt, defaultCycle, offsetOf, type Block } from './cycles'
...
const props = withDefaults(
  defineProps<{
    ...
    shape?: string
    /** identifiant de couleur du personnalisateur, ou un hex `#rrggbb` */
    color?: string
...
const ink = computed(
  () => COLOR_BY_ID.get(props.color)?.hex ?? (/^#[0-9a-f]{3,8}$/i.test(props.color) ? props.color : '#0a0a0c')
)
...
function redrawFrozen() {
  ...
  /*
   * En mode vignette l'horloge n'a jamais tourne (clock = 0), donc `clock` et
   * `frozenAt` coincident. En usage chat, le fige arrive APRES des changements
   * d'etat dates a l'horloge : echantillonner `frozenAt` (0) relirait une date
   * anterieure au dernier fondu et l'ease-out extrapolerait la silhouette.
   * Figer l'image courante (`clock`) est correct dans les deux cas.
   */
  frame.value = engine.sample(Math.max(clock, props.frozenAt))
  triggerRef(frame)
}
 ...
 *
 * Le chat, lui, bascule `frozenAt` en pleine vie du composant : figer doit
 * ARRETER la boucle (sinon l'avatar d'un message termine tourne pour toujours),
 * et degeler doit la relancer (le montage fige au chargement de l'historique ne
 * l'a jamais demarree).
 */
watch(() => props.frozenAt, (at) => {
  if (at === undefined) {
    if (!raf) {
      last = 0
